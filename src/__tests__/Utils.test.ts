import { getName, getNameOrUnknown, cleanTsObj, fetchDescriptionFromComment, getExample, hasExample, hasDropdown, getDropdown, stripTags } from '../Utils';
...
describe('Test getName', () => {
    ...
    test('Should throw on invalid input', () => {
        expect(() => getName(undefined)).toThrow('Failed to parse name from null value')
...
describe('Test getName', () => {
    test('Should remove return descriptions', () => {
        expect(fetchDescriptionFromComment("this is a comment")).toEqual("this is a comment")
    })
    test('Should stop on @param', () => {
        expect(fetchDescriptionFromComment("this is a comment  @param")).toEqual("this is a comment")
    })
    test('Should trim strings', () => {
        expect(fetchDescriptionFromComment("     \n\n \t\t this is a comment     \n\n \t\t     ")).toEqual("this is a comment")
    })
...
describe('Test getExample', () => {
    test('should return value of', () => {
        expect(getExample('dont keep #sample:foo bar', 'string')).toEqual({ en: "foo bar" })
    })
    test('numbers should be numbers', () => {
        expect(getExample('dont keep #sample:42', 'number')).toEqual(42)
    })
    test('missing tag should throw', () => {
        expect(() => getExample('dont keep', 'string')).toThrow('Missing example tag #sample: in dont keep')
    })
})
...
describe('Test getDropdown', () => {
    test('should return value of', () => {
        expect(getDropdown('dont keep #dropdown:{"foo":"bar"}')).toEqual({ "foo": "bar" })
    })
    test('missing tag should throw', () => {
        expect(() => getDropdown('dont keep')).toThrow('Missing example tag #dropdown: in dont keep')
    })
    test('invalid json should throw', () => {
        expect(() => getDropdown('dont keep #dropdown:{"foo":"bar"')).toThrow("Failed to parse");
    })
})
