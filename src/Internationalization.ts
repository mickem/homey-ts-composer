...
function applyValue(common: any, localeObj: any, realObj: any, language: string, key: string, desc: string) {
    try {
        if (localeObj && localeObj[key]) {
            realObj[key][language] = localeObj[key];
        } else if (realObj[key] && realObj[key].en && realObj[key].en.startsWith("$common.")) {
            if (common[realObj[key].en.substr(8)]) {
                realObj[key][language] = common[realObj[key].en.substr(8)];
            }
        } else {
            console.error(`Ignoring missing translation ${language} ${desc} (${key})`);
        }
    } catch (exception) {
        console.error(`Failed to translate key:  ${language} ${desc} (${key})`, exception);
    }
}
...
export function updateBaseLocale(result: IApp, localData: ILocaleData): ILocaleData {
    ...
    if (result.flow && result.flow.triggers) {
        for (const t of result.flow.triggers) {
            ...
            for (const token of t.tokens) {
                ...
                collector.collectAndSet(token.title.en, triggerData[t.id][token.name], 'title');
                if (token.type === "string") {
                    collector.collectAndSet(fetchExample(token, `${t.id} ${token.name}`), triggerData[t.id][token.name], 'example');
                }
    ...
    for (const t of result.flow.actions) {
        ...
        for (const arg of t.args) {
            ...
            collector.collectAndSet(arg.title.en, actionData[t.id][arg.name], 'title');
            if (arg.type === "text") {
                collector.collectAndSet(fetchExample(arg, `${t.id} ${arg.name}`), actionData[t.id][arg.name], 'example');
            }
...
export function addLocaleToResult(code: string, result: IApp, localData: ILocaleData): IApp {
    if (localData.flows && localData.flows.triggers) {
        result.flow.triggers = result.flow.triggers.map(trigger => {
            ...
            if (!trigger) {
                console.error(`Invalid data when parsing trigger:`, result.flow.triggers);
                throw new Error(`Invalid data when parsing trigger`);
            }
            applyValue(localData.common, localeTrigger, trigger, code, 'title', `${code} ${trigger.id}`);
            trigger.tokens.forEach(token => {
                const localeToken = localeTrigger[token.name];
                applyValue(localData.common, localeToken, token, code, 'title', `${code} ${trigger.id} / ${token.name}`);
                applyValue(localData.common, localeToken, token, code, 'example', `${code} ${trigger.id} / ${token.name}`);
            });
            return trigger;
        });
    }
    if (localData.flows && localData.flows.actions) {
        result.flow.actions = result.flow.actions.map(action => {
            if (!action) {
                console.error(`Invalid data when parsing trigger:`, result.flow.actions);
                throw new Error(`Invalid data when parsing trigger`);
            }
            ...
            applyValue(localData.common, localeAction, action, code, 'title', `${code} ${action.id}`);
            action.args.forEach(arg => {
                const localeArg = localeAction[arg.name];
                applyValue(localData.common, localeArg, arg, code, 'title', `${code} ${action.id} / ${arg.name}`);
                applyValue(localData.common, localeArg, arg, code, 'example', `${code} ${action.id} / ${arg.name}`);
            });
            return action;
        });
    }
